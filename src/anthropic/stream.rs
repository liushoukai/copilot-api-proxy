...
/// 将一个 OpenAI chunk 翻译为若干 Anthropic SSE 事件
pub fn translate_chunk(
    ...
    if let Some(content_str) = delta.get("content").and_then(|v| v.as_str()) {
        if !content_str.is_empty() {
            ...
            if state.is_tool_block_open() {
                events.push(StreamEvent::ContentBlockStop { index: state.content_block_index });
            ...
            events.push(StreamEvent::ContentBlockDelta {
                index: state.content_block_index,
                delta: ContentDelta::TextDelta { text: content_str.to_string() },
            });
        }
    }
    ...
    if let Some(tool_calls) = delta.get("tool_calls").and_then(|v| v.as_array()) {
        for tc in tool_calls {
            ...
            let tc_name = tc.get("function").and_then(|f| f.get("name")).and_then(|v| v.as_str()).map(String::from);
            let tc_args = tc.get("function").and_then(|f| f.get("arguments")).and_then(|v| v.as_str()).map(String::from);

            // 新 tool call 开始（有 id 和 name）
            if let (Some(id), Some(name)) = (tc_id, tc_name) {
                ...
                state.tool_calls.insert(tc_index, (id.clone(), name.clone(), block_index));
                events.push(StreamEvent::ContentBlockStart {
                    index: block_index,
    ...
    if let Some(finish_reason) = choice.finish_reason.as_deref() {
        ...
        events.push(StreamEvent::MessageDelta {
            ...
            usage: StreamUsage { input_tokens, output_tokens, cache_read_input_tokens: cache_read },
        });
